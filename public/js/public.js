$(document).ready(function () {
	...
	host = host+':8080/GSHT/giamsathanhtrinhoto/';
	//alert(host);
	getAllBienso();
	//------------------------------------------//
	...
	function thongtinoto() {
		...
		var url = host + '/include/' + 'function.php?service='+'thongtinoto'+'&bienso='+bienso+'&tutime='+tutime+'&dentime='+dentime;
		//alert(url);
		$.ajax({
			type: "POST",
			url: url,
			success: function(data) {
				...
				var source = 
				{
					...
					datafields:
					[
						{ name: 'log_gps_speed', type: 'string' },
						{ name: 'log_time', type: 'string' },
						{ name: 'log_latitude', type: 'string' },
						{ name: 'log_longitude',type: 'string' },
						{ name: 'log_km_meter',type: 'string' }
					],
					datatype: "array",
					pagesize: 20
				};
				...
				$("#panel-sukien").jqxGrid({
					...
					altrows: true,
					columns: [
						{ text: 'Vận tốc', datafield: 'log_gps_speed', width: 150 },
							  	{ text: 'Thời gian', datafield: 'log_time', width: 150, cellsalign: 'left' },
							  	{ text: 'Vĩ độ', datafield: 'log_latitude', width: 80, cellsalign: 'left' },
								{ text: 'Kinh độ', datafield: 'log_longitude', width: 80, cellsalign: 'left' },
								{ text: 'Lý trình', datafield: 'log_km_meter', width: 80, cellsalign: 'left' }
					]
				});
			}
		});
	...
	function regUser() {
		...
		url = host + '/include/' + 'function.php?service='+'reguser'+'&username='+username+'&password='+password+'&donvi='+dv;
		$.ajax({
			url: url,
			type: "POST",
			url: url,
	...
	function regDriver() {
		...
		url = host + '/include/' + 'function.php?service='+'regdriver'+'&username='+username+'&password='+password+'&bienso='+bienso; 
	...
	function getAllBienso() {
		...
		url = host + '/include/' + 'function.php?service='+'getallbienso';
		$.ajax({
			type: "POST",
			url: url,
			success: function(data) {
	...
	function getBiensoByDonvi5(donvi) {
		...
		url = host + '/include/' + 'function.php?service='+'getbiensobydonvi'+'&donvi='+donvi;
	...
	function getGaByDonVi(donvi) {
		...
		url = host + '/include/' + 'function.php?service='+'getgabydonvi'+'&donvi='+donvi;
		$.ajax({
			type: "POST",
			url: url,
			success: function(data) {
	...
	function getVitriMoinhatByArrBienSo(arrBS) {
		...
		url = host + '/include/' + 'function.php?service='+'getvitritaumoinhatbyarrbienso'+'&arrBS='+jsonString;
		$.ajax({
			type: "POST",
			url: url,
			success: function(data) {
				...
				var lonCenter = json[length2].log_longitude;
				var latCenter = json[length2].log_latitude;
				var myLatlng = new google.maps.LatLng(latCenter,lonCenter);
				var mapOptions = {
				    zoom: 10,
				    center: myLatlng
				...
				for(var i = 0; i < length; i++) {
					//console.log(json[i].ga_kinhdo);
					arrLat.push(json[i].log_latitude);
					arrLon.push(json[i].log_longitude);
					arrSohieu.push(json[i].plate_number);
					arrTime.push(json[i].log_time);
					arrLyTrinh.push(json[i].log_km_meter);
					arrSpeed.push(json[i].log_gps_speed);
					arrSpeedDautruc.push(json[i].log_wheel_speed);
					if(json[i].log_gps_speed > 80) {
						image = '';
						image = image + host+'/public/images/train-icon16.png';
					}
					else if(json[i].log_gps_speed <= 80) {
						image = '';
	...
	function getVitriMoinhatByBienSo(bienso) {
		...
		url = host + '/include/' + 'function.php?service='+'getvitritaumoinhatbybienso'+'&bienso='+bienso;
		$.ajax({
			type: "POST",
			url: url,
			success: function(data) {
				...
				var time = json[0].log_time;
				var lytrinh = json[0].log_km_meter;
				var gps_speed = json[0].log_gps_speed;
				var wheel_speed = json[0].log_wheel_speed;
				var kinhdo = json[0].log_longitude;
				var vido = json[0].log_latitude;
				var imei = json[0].log_imei;
				var html = '<div style="width: 350px;">';
				html += '<table>';
				...
				var lat = json[0].log_latitude;
				var lon = json[0].log_longitude;
